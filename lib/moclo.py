...
def cds_check(gene_id,seq):
    ...
    for test in basic_tests:
        if not test(seq):
            raise ValueError(test.__name__)
    return "Clear"

def input_checker(gene_id,seq):
    seq = seq.upper()
    if "A"*11 in seq:
        raise ValueError('"A" homopolymer of 11 or more basepairs')
    elif "T"*11 in seq:
        raise ValueError('"T" homopolymer of 11 or more basepairs')
    elif "G"*11 in seq:
        raise ValueError('"G" homopolymer of 11 or more basepairs')
    elif "C"*11 in seq:
        raise ValueError('"C" homopolymer of 11 or more basepairs')
    elif "GGTCTC" in seq:
        raise ValueError('Contains bsaI site (GGTCTC)')
    elif "GAGACC" in seq:
        raise ValueError('Contains bsaI site (GAGACC)')
    elif "GAAGAC" in seq:
        raise ValueError('Contains bbsI site (GAAGAC)')
    elif "GTCTTC" in seq:
        raise ValueError('Contains bbsI site (GTCTTC')
    elif len(seq) < 10:
        raise ValueError('Sequence too small (<10bp)')
    elif len(seq) > 20000:
        raise ValueError('Sequence too large (>20kbp)')
    else:
        return "Clear"
...
def sequence_input(gene_id,seq,part_type):
    ...
    if part_type == 'cds':
        try:
            ...
            print(e)
            out += "{} failed on {}".format(gene_id,e)
            return out
    ...
    out += "{} successfully checked and ready to submit".format(gene_id)
    return [frags, seq, out]
